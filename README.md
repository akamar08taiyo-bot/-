...
| `slot.html` | スロット差枚アナライザー（みんレポ等のデータ分析） |
...
みんレポ等の日別差枚データを取り込み、店トータルから「何を打てば勝ちやすいか」まで出す単体アプリ
（Firebase 非依存。データはブラウザの localStorage にだけ保存し、CSVで書き出し／再取り込みできる）。

**取り込み**（`src/slot/parse.ts`）— 見出し名（台番・機種・G数・差枚・BB・RB）で列を探すので列順が違っても読める。
- みんレポのページを全選択コピー → 貼り付け（日付・店名はタイトル「2026/9/22(火) スペース666 | …」から自動で読む）
- ページのHTML／保存したHTMLファイル（複数まとめて可）／このツールのCSV
- まとめて保存したいときは自分のPCで `python3 tools/minrepo_fetch.py --tag スペース666 --from 2026-09-01 --to 2026-09-30`
  を実行し、できたHTMLを「ファイルを選ぶ」で一括取り込み

**分析**（`src/slot/analyze.ts`）
| タブ | 内容 |
| --- | --- |
| 日別 | 店トータル差枚・勝率・出率・高設定推測台数、機種別／末尾別、全台系の疑い、並び（連続プラス区間）、全台一覧 |
| 期間・月間 | 日別店トータルのグラフ、機種ランキング（少数サンプルを店平均へ寄せた補正勝率順）、日付末尾・曜日・台番末尾・設置台数別、前日差枚→翌日成績、隣台との相関、台番別通算 |
| 狙い目 | 打つ日の条件（日付末尾・曜日・ゾロ目）に合う過去日から「機種×条件日の機種×末尾×台」を縮小推定し、推定勝率・期待差枚の順に台を並べる |
| 検証 | ウォークフォワード・バックテスト。各日を「その日より前のデータだけ」で予測し、5つの選び方（総合モデル／機種／末尾／前日凹み／前日勝ち）を店平均と比較 |
| 発信 | X用短文（280字に自動で収める）・まとめ画像（1200×675 PNG）・note用長文、狙い目で記録した予想の自動答え合わせと通算実績、月間レポート記事（`<!-- ここから有料 -->` で無料/有料を分けたMarkdown） |

出率は `(G数×3 + 差枚) / (G数×3)` の近似、高設定推測は「G数・出率がしきい値以上」の粗い目安（しきい値は画面で変更可）。

発信用の出力（`src/slot/publish.ts`）は全台一覧を載せず集計と上位だけを使い、出典と免責を必ず付ける。

**検証** — `npx tsx scripts/verify-slot.ts` で取り込み・集計・推奨・バックテスト・発信用出力の26項目を確認する。
傾向を仕込んだ架空データ（`src/slot/demo.ts`、実在店舗のデータではない）で、仕込んだ傾向を分析が見つけられるかも確かめている。
